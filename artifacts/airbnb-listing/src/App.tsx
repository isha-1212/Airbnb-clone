...
function RatingReviews() {
  const [showAll,setShowAll]=useState(false); const [topic,setTopic]=useState('');
  return <section id="reviews" className="rating-section"><div className="rating-hero">4.95</div><small>Guest favourite</small><p className="rating-desc">This home is a guest favourite based on ratings, reviews and reliability</p><button className="link" data-testid="button-how-reviews">How reviews work</button><div className="breakdown"><div className="break-col"><div className="break-label">Overall rating</div>{[5,4,3,2,1].map((n,i)=><div className="bar-row" key={n}>{n}<span className="bar" style={{width:i===0?'95%':i===1?'8%':'2%',background:i===0?'#222':'#ddd'}}/></div>)}</div>{[['Cleanliness','5.0',Sparkles],['Accuracy','5.0',Check],['Check-in','5.0',KeyRound],['Communication','5.0',Bell],['Location','4.8',MapPin],['Value','4.8',Tag]].map(([name,score,Icon])=><div className="break-col" key={name as string}><div className="break-label">{name as string}</div><div className="cat-score">{score as string}</div><Icon className="cat-icon" size={22}/></div>)}</div><div className="topics">{topicLabels.map(label=><button className={`topic ${topic===label?'active':''}`} key={label} onClick={()=>setTopic(topic===label?'':label)} data-testid={`button-topic-${label.split(' ')[0].toLowerCase()}`}>{label}</button>)}</div><div className="review-grid">{reviews.slice(0,showAll?4:2).map((review,index)=><article className="review" key={review.name}><div className="review-head"><div className="review-avatar">{review.name[0]}</div><div><div className="review-name">{review.name}</div><div className="muted">{review.tenure}</div></div></div><div className="stars">★★★★★ <span className="muted">{review.time}</span></div><p>{review.text}</p>{index===1&&<button className="link" data-testid={`button-review-more-${index}`}>Show more</button>}</article>)}</div><button className="outline-btn reviews-more" onClick={()=>setShowAll(!showAll)} data-testid="button-reviews-more">{showAll?'Show fewer reviews':'Show all 19 reviews'}</button></section>;
}

function LocationHost() {
  return <><section id="location"><h2>Where you'll be</h2><div className="muted">Candolim, Goa, India</div><div className="map"><div className="map-controls"><button aria-label="Search map" data-testid="button-map-search"><Search size={16}/></button><span><button aria-label="Zoom in" data-testid="button-map-plus"><Plus size={15}/></button><button aria-label="Zoom out" data-testid="button-map-minus"><Minus size={15}/></button></span></div><div className="pin"><House size={19}/></div></div><div className="muted">Exact location will be provided after booking.</div><div className="neighborhood"><h2>Neighbourhood highlights</h2><p>Located in the heart of Candolim, Amor de Goa offers a peaceful stay with easy access to beaches, cafes, and popular attractions.</p><button className="show-more" data-testid="button-neighborhood-more">Show more <ArrowRight size={14}/></button></div></section><section id="host" className="host-section"><h2>Meet your host</h2><div className="host-layout"><div><div className="host-card"><div><div className="avatar">MIRASHYA</div><div className="muted">Host</div></div><div className="host-stats"><b>1,463</b><span>Reviews</span><b>4.68★</b><span>Rating</span><b>2</b><span>Years hosting</span></div></div><div className="facts"><MapPin size={14}/> Born in the 80s<br/><GraduationCap size={14}/> Where I went to school: NICMAR GOA</div></div><div><h3>Co-Hosts</h3><div className="cohosts">{cohosts.map((name)=><div className="cohost" key={name}><span className="cohost-avatar">{name[0]}</span>{name}</div>)}</div><div className="host-detail"><b>Response rate: 100%</b><br/><span className="muted">Responds within an hour</span><br/><button className="outline-btn message" data-testid="button-message-host">Message host</button></div></div></div></section></>;
}

function ThingsNearby() {
  const [page,setPage]=useState(0);
  return <><section className="safety"><ShieldCheck size={24}/><span>To help protect your payment, always use Airbnb to send money and communicate with hosts.</span></section><section><h2>Things to know</h2><div className="things">{[['Cancellation policy',['Free cancellation before 17 October.','Cancel before check-in on 18 October for a partial refund.']],['House rules',['Check-in after 2:00 pm','Checkout before 11:00 am','3 guests maximum']],['Safety & property',['Carbon monoxide alarm not reported','Smoke alarm not reported','Exterior security cameras on property']]].map(([title,items])=><div className="thing" key={title as string}><h3>{title as string}</h3><ul>{(items as string[]).map(item=><li key={item}>{item}</li>)}</ul><button className="link" data-testid={`button-learn-${title}`}>Learn more</button></div>)}</div></section><section><div className="nearby-head"><h2>More stays nearby</h2><div className="carousel-controls"><span>{page+1} / 2</span><button onClick={()=>setPage(Math.max(0,page-1))} aria-label="Previous stays" data-testid="button-nearby-prev"><ChevronLeft size={16}/></button><button onClick={()=>setPage(Math.min(1,page+1))} aria-label="Next stays" data-testid="button-nearby-next"><ChevronRight size={16}/></button></div></div><div className="nearby-grid">{nearby.map(([title,price,rating,url])=><article className="stay" key={title}><img src={url} alt={title}/><div className="stay-title">{title}</div><div className="stay-meta">{price} · ★ {rating}</div></article>)}</div></section></>;
}
...
function ListingPage({goTour,openLightbox}: {goTour:()=>void;openLightbox:(index:number)=>void}) {
  const [active,setActive]=useState('photos');
  const [selected,setSelected]=useState<DateRange>([18,23]);
  useEffect(()=>{const sections=['photos','amenities','reviews','location'].map(id=>document.getElementById(id)).filter(Boolean) as HTMLElement[];const observer=new IntersectionObserver((entries)=>{const visible=entries.filter(e=>e.isIntersecting).sort((a,b)=>b.intersectionRatio-a.intersectionRatio)[0];if(visible)setActive(visible.target.id)}, {rootMargin:'-100px 0px -55% 0px',threshold:[0,.15,.5]});sections.forEach(s=>observer.observe(s));return()=>observer.disconnect()},[]);
  const scrollTo=(id:string)=>document.getElementById(id)?.scrollIntoView({behavior:'smooth'});
  return <div className="page"><Header/><main className="container"><Hero onPhotos={goTour}/></main><SectionNav active={active} scrollTo={scrollTo} selected={selected}/><main className="container"><div className="columns"><div className="content"><Summary/><DescriptionAndSleep/><Amenities/><Calendar selected={selected} onSelect={setSelected}/><RatingReviews/><LocationHost/><ThingsNearby/></div><BookingCard selected={selected}/></div><footer className="footer"><span>© 2026 Airbnb clone · Candolim, India</span><span>Privacy · Terms · Support</span></footer></main></div>;
}
